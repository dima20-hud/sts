function js2_f11()
{
document.getElementById("pic1").src="Вуди.ipj";
document.getElementById("body1").style.backgroundColor="blue";
document.getElementById("body2").style.backgroundColor="yellow";
} 
function js2_f12()
{
if(document.getElementById("sl1").selectedIndex==0)
document.getElementById("body1").style.backgroundColor="green";
 
if(document.getElementById("sl2").selectedIndex==1)
document.getElementById("body2").style.backgroundColor="red";
 
if(document.getElementById("sl3").selectedIndex==2)
document.getElementById("body3").style.backgroundColor="pink";
}
function js2_f13()
{
document.getElementById("area1").value="Привет, Мир!!!";
 
}
